...
const stripTerminatorColor = text => text.replace(/\x1B\[\d+m/g, "");
...
describe("toContainText matcher", () => {
	...
	it("returns a message that contains the source line if no match", () => {
		...
		expect(stripTerminatorColor(result.message())).toContain(`expect(element).toContainText("text to find")`)
	})
	...
	it("returns a message that contains the source line if negated match", () => {
		...
		expect(stripTerminatorColor(result.message())).toContain(`expect(element).not.toContainText("text to find")`)
	})
	...
	it("returns a message that contains the actual text", () => {
		...
		expect(stripTerminatorColor(result.message())).toContain(`Actual text: "text to find`)
	})
})
